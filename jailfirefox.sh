#!/bin/bash
#---------------------------------------------------------------------------
#title       :jailfirefox.sh
#description :sets environment to run legacy firefox & java to access UBISS from localhost AND jails one chosen  existing user with limited commands when they ssh to this computer
#author      :Paolo V.M. Distefano `pvd`
#date        :20180311
#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# environment.
#
APPS="/bin/bash /bin/cat /bin/cp /bin/grep /bin/ls /bin/mkdir /bin/more /bin/mv /bin/pwd /bin/rm /bin/rmdir /usr/bin/du /usr/bin/head /usr/bin/id /usr/bin/less /usr/bin/ssh /usr/bin/scp /usr/bin/tail /usr/bin/rsync /bin/hostname /bin/ps /bin/df /bin/top /bin/htop /bin/free /bin/ping"
...
" >> /etc/ssh/sshd_config
  ...
  echo
  #pvd / modificato per attivazione sshd dallo script stesso
  #echo "** please restart ssh daemon, then re-run script again"

  echo "** enabling and starting sshd which is required for this jail"\n
  echo "** please insert your current user password to allow sudo to run"\n
  echo "** if you do not have permissions for sudo, please obtain such credentials and run this script again"
